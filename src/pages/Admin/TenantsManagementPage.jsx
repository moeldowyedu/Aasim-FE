...
const TenantsManagementPage = () => {
  ...
  }, [currentPage]); // Add debounced search dependency in future
  ...
  const getStatColor = (colorName) => {
    ...
    return theme === 'dark'
      ? { bg: pal.lightBg, text: pal.lightText }
      : { bg: pal.paleBg, text: pal.paleText };
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
          ...
          {stats.map((stat, index) => {
            ...
            return (
              <Card key={index} hover>
                ...
                <p className="text-green-500 text-xs font-semibold">{stat.change}</p>
              </Card>
            );
          })}
        </div>
        ...
        <Card>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-2">
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:border-purple-500 ${theme === 'dark' ? 'bg-gray-900 border-gray-700 text-white placeholder-gray-500' : 'bg-white border-slate-200 text-slate-900 placeholder-slate-400'}`}
                />
              </div>
            </div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className={`w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:border-purple-500 appearance-none ${theme === 'dark' ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
              >
          ...
          <div className="mt-4 flex justify-end">
            <button className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-slate-200 hover:bg-slate-300 text-slate-700'}`}>
        ...
        <Card padding="none" className="overflow-hidden">
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className={theme === 'dark' ? 'bg-gray-900/50 text-gray-400' : 'bg-slate-50 text-slate-500'}>
                  <tr>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">Tenant</th>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">Admin</th>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">Plan & Trial</th>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">Start Date</th>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">Status</th>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">Users</th>
                    <th className="text-left py-3 px-4 text-xs font-bold uppercase">End Date</th>
                    <th className="text-right py-3 px-4 text-xs font-bold uppercase">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                  {tenants.map((tenant) => (
                    <tr key={tenant.id} className={`${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-slate-50'} transition-colors`}>
                      ...
                      <td className="py-4 px-4">
                        {tenant.tenant_admin ? (
                          <div className="flex items-center space-x-2">
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center overflow-hidden ${theme === 'dark' ? 'bg-gray-700' : 'bg-slate-200'}`}>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center justify-end space-x-1">
                          <button className={`p-1.5 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-slate-100'}`} title="View Details">
                            <Eye className="w-3.5 h-3.5 text-blue-400" />
                          </button>
                          <button
                            onClick={() => handleOpenModal(tenant)}
                            className={`p-1.5 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-slate-100'}`} title="Manage Subscription">
                            <Settings className="w-3.5 h-3.5 text-purple-400" />
                          </button>
                          <button className={`p-1.5 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-slate-100'}`} title="Login As">
                            <LogIn className="w-3.5 h-3.5 text-green-400" />
                          </button>
                          {tenant.status === 'Suspended' ? (
                            <button
                              onClick={() => handleAction('activate', tenant.id)}
                              className={`p-1.5 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-slate-100'}`}
                              title="Activate">
          ...
          {/* Pagination */}
          {tenants.length > 0 && (
            <div className={`px-6 py-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-800 ${theme === 'dark' ? 'bg-gray-900/50' : 'bg-slate-50'}`}>
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg transition-colors text-sm font-semibold ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 disabled:text-gray-600 text-white' : 'bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 disabled:opacity-50'}`}
                >
                  Previous
                </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className={`w-full max-w-md p-6 rounded-xl shadow-2xl ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-slate-200'}`}>
            <div className="flex items-center justify-between mb-6">
              <h3 className={`text-xl font-bold ${textPrimary}`}>Manage Subscription</h3>
              <button
                onClick={() => setIsModalOpen(false)}
                className={`p-1 rounded-full ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-slate-100'} transition-colors ${textSecondary}`}>
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className={`p-3 rounded-lg border ${theme === 'dark' ? 'bg-gray-900 border-gray-700' : 'bg-slate-50 border-slate-200'} ${textPrimary}`}>
                  {selectedTenant?.name}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full p-3 rounded-lg border focus:ring-2 focus:ring-purple-500 outline-none ${theme === 'dark' ? 'bg-gray-900 border-gray-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
                />
              ...
              <div className="flex items-center justify-end space-x-3 mt-6">
                <button
                  onClick={() => setIsModalOpen(false)}
                  className={`px-4 py-2 rounded-lg font-medium ${theme === 'dark' ? 'text-gray-300 hover:bg-gray-700' : 'text-slate-600 hover:bg-slate-100'}`}>
                  Cancel
                </button>
                <button
