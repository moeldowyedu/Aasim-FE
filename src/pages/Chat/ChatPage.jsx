...
const ChatPage = () => {
  ...
  const renderMessageStatus = (status) => {
    switch (status) {
      case 'sent':
        return <Check className="w-3 h-3 text-secondary-600" />;
      case 'delivered':
        return <CheckCheck className="w-3 h-3 text-secondary-600" />;
      case 'read':
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredConversations.map((conv) => (
              <div
                ...
                className={`p-4 border-b border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors ${selectedConversation?.id === conv.id ? 'bg-primary-50 border-l-4 border-l-primary-600' : ''
                  }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center">
                      <span className="text-secondary-900 font-bold text-sm">{conv.avatar}</span>
                    </div>
                    {conv.online && (
                  ...
                  {/* Unread Badge */}
                  {conv.unread > 0 && (
                    <div className="flex-shrink-0 w-5 h-5 bg-primary-600 text-secondary-900 rounded-full flex items-center justify-center text-xs font-bold">
                      {conv.unread}
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center">
                      <span className="text-secondary-900 font-bold text-sm">{selectedConversation.avatar}</span>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selectedConversation.online && (
                        <>
                          <Circle className="w-1 h-1 fill-gray-400 text-secondary-600" />
                ...
                >
                  ...
                  {!message.isOwn && (
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-purple-600 flex items-center justify-center flex-shrink-0">
                      <span className="text-secondary-900 font-bold text-xs">{selectedConversation.avatar}</span>
                    </div>
                  )}
                  ...
                  <div className={`max-w-md ${message.isOwn ? 'items-end' : 'items-start'} flex flex-col`}>
                    <div
                      className={`rounded-2xl px-4 py-2 ${message.isOwn
                        ? 'bg-gradient-to-r from-primary-600 to-primary-700 text-secondary-900'
              ...
              <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent pr-12"
                  />
                  <button
                    type="button"
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-50 rounded-lg transition-colors"
                  >
                ...
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-primary-600 to-primary-700 text-secondary-900 rounded-xl font-semibold hover:shadow-lg transition-shadow disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
